# added my own data set the same as deepglobe
import numpy as np
...
def get_mean_iou(conf_mat, dataset):
    ...
    """
    IoU = getIoU(conf_mat)
    if dataset == "deepglobe" or dataset == "river_segment" or dataset=="deepglobe_river":
...
def get_freq_iou(conf_mat, dataset):
    ...
    if dataset == "deepglobe" or dataset == "river_segment" or dataset=="deepglobe_river":
        # print(f'in get freq iou freq[:]={freq[:]}\n\n')
        return (IoU[1:] * freq[1:]).sum() / (freq[1:].sum() + 1e-10)
    elif dataset == "cityscapes":
        return (IoU * freq).sum()
    else:
        raise "Not implementation for dataset {}".format(dataset)
...
def get_overall_iou(conf_mat, dataset):
    ...
    Returns:
        float: overall iou
    """
    if dataset in ["deepglobe", "cityscapes", "river_segment", "deepglobe_river"]:
