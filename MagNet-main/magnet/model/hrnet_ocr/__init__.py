...
from .config import blocks_dict, config_hrnet_w18_ocr, config_hrnet_w48_ocr, config_hrnet_w32
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, n_classes, full_os=True):
        ...
        # stem net
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1 if full_os else 2, padding=1, bias=False)
        self.bn1 = BatchNorm2d(64, momentum=BN_MOMENTUM)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=1 if full_os else 2, padding=1, bias=False)
        ...
        self.aux_head = nn.Sequential(
            ...
            nn.ReLU(inplace=relu_inplace),
            nn.Conv2d(last_inp_channels, n_classes, kernel_size=1, stride=1, padding=0, bias=True),
        )
    ...
    def forward(self, x, return_auxilary=False):
        ...
        if return_auxilary:
            return out, out_aux
        # print((out + 0.4 * out_aux).shape)
        
        # print((out + 0.4 * out_aux)[0,:,:,:].argmax(axis=0))
...
class HighResolutionNetSimple(nn.Module):
    config = None

    def __init__(self, n_classes, full_os=True):

        extra = self.config["EXTRA"]
        super(HighResolutionNetSimple, self).__init__()
    ...
    def forward(self, x, return_auxilary=False):
        ...
        # print(out_aux.shape)
        feats = F.interpolate(feats, size=(H, W), mode="bilinear", align_corners=ALIGN_CORNERS)
        # print(out_aux.shape)
        # print(feats[0,:,:,:].argmax(axis=0))
        # print('------------')
...
class HRNetW18(HighResolutionNetSimple):
    config = config_hrnet_w18_ocr

class HRNetW32(HighResolutionNetSimple):
    config = config_hrnet_w32

class HRNetW48(HighResolutionNetSimple):
    config = config_hrnet_w48_ocr