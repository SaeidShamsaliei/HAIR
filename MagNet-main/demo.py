    ...
    for batch_idx in range(n_batches):
        ...
        with torch.no_grad():
            if another is None:
                ...
                preds += [torch.softmax(model(batch, another[batch_idx *
                                        sub_batch_size: max_index]), dim=1)]
...
def main():
    ...
    for scale in opt.scales:
        patch_coords += [torch.tensor(get_patch_coords(scale,
                                      opt.crop_size)).to(device)]
    ...
    # Blur operator
    median_blur = MedianBlur(kernel_size=(
        opt.smooth_kernel, opt.smooth_kernel)).to(device)
    median_blur.eval()

    # Loading image
    image = cv2.cvtColor(cv2.imread(
        opt.image, cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)
    ...
    for idx, (ratios, scale) in enumerate(zip(patch_coords, opt.scales)):
        ...
        if idx == 0:
            ...
            # Get prediction
            final_output = get_batch_predictions(
                model, 1, image_patches[0:1].to(device))
        ...
        final_output = F.interpolate(
            final_output, scale[::-1], mode="bilinear", align_corners=False)
        ...
        patch_uncertainty = roi_align(uncertainty, [coords], output_size=(
        ...
        scale_early_preds = get_batch_predictions(
            model, sub_batch_size, scale_image_patches.to(device))

        # Get coarse preds (with coords and final_output)
        coarse_preds = roi_align(
            final_output, [coords[selected_patch_ids]], output_size=(
                opt.input_size[1], opt.input_size[0])
        )
        ...
        uncertainty_score = F.interpolate(
            uncertainty, scale[::-1], mode="bilinear", align_corners=False)
        ...
        error_score = F.interpolate(error_score, size=(
            opt.input_size[1], opt.input_size[0]))
        with torch.no_grad():
        ...
        if opt.n_points > 1.0:
            n_points = min(
                int(opt.n_points), scale[0] * scale[1] * len(selected_patch_ids) / len(coords))
        else:
            n_points = int(scale[0] * scale[1] * opt.n_points *
                           len(selected_patch_ids) / len(coords))
        ...
        fine_pred = point_sample(
            fine_pred, error_point_coords, align_corners=False)

        if opt.n_patches > 0:
            # Apply mask
            sample_mask = (
                point_sample(mask.type(torch.float).unsqueeze(0).unsqueeze(
                    0), error_point_coords, align_corners=False)
                .type(torch.bool)
                .squeeze()
            )
    ...
    if opt.save_pred:
        ...
        for scale, pred in zip(opt.scales, intermediate_preds):
            ...
            pred_path = os.path.join(
                opt.save_dir, image_name, "{}x{}.jpg".format(scale[0], scale[1]))
