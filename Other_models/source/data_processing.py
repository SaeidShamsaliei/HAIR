...
class TrainingImage:
    ...
    def write_data_to_raster(self, output_filepath):
        self._write_array_to_raster(
            output_filepath, self.data, self.geo_transform)
    ...
    def write_labels_to_raster(self, output_filepath):
        ...
        self._write_array_to_raster(
            output_filepath, self.labels, geo_transform)
...
def create_pointer_files(data_path, output_folder, train_size=0.6, valid_size=0.2, test_size=0.2, shuffle=True,
    ...
    image_paths = glob.glob(os.path.join(
        data_path, "images", "*." + file_ending))
    if sample_rate < 1:
        image_paths = image_paths[:int(len(image_paths)*sample_rate)]

    if shuffle:
        random.shuffle(image_paths)
    label_paths = [path.replace("images", "labels").replace(
        "tiny_labels", "tiny_images") for path in image_paths]
    os.makedirs(output_folder, exist_ok=True)
    # Make training file
    with open(os.path.join(output_folder, "train.txt"), "w+") as f:
        pairs = [image_paths[i] + ";" + label_paths[i]
                 for i in range(int(train_size*len(image_paths)))]
        f.write("\n".join(pairs))
    # Make validation file
    with open(os.path.join(output_folder, "valid.txt"), "w+") as f:
        end_index = int(train_size * len(image_paths)) + \
            int(valid_size * len(image_paths))
        pairs = [image_paths[i] + ";" + label_paths[i]
                 for i in range(int(train_size * len(image_paths)), end_index)]
        f.write("\n".join(pairs))
    # Make test file
    with open(os.path.join(output_folder, "test.txt"), "w+") as f:
        start_index = int(train_size * len(image_paths)) + \
            int(valid_size * len(image_paths))
        pairs = [image_paths[i] + ";" + label_paths[i]
...
def divide_image(image_filepath, label_filepath, image_size=512, do_overlap=False, do_crop=False,
    ...
    """
    add_filter_channel: determines if we need to add filters as additional channels 
    """
    ...
    image_matrix = np.array(Image.fromarray(
        image_matrix.squeeze().astype(np.uint8)).convert('L'))
    image_ds = None
    ...
    if label_ds.GetGeoTransform() != geo_transform:
        raise Exception(
            f"The geo transforms of image {image_filepath} and label {label_filepath} did not match")
    ...
    for shape_0 in shape_0_indices:
        for shape_1 in shape_1_indices:
            if do_crop:
                ...
                labels = label_matrix[shape_0:shape_0 +
                                      image_size, shape_1:shape_1 + image_size]
            ...
            if do_crop:
                # East
                label_geo_transform[0] += (shape_1 +
                                           image_size//4) * geo_transform[1]
                # North
                label_geo_transform[3] += (shape_0 +
                                           image_size//4) * geo_transform[5]
            else:
                label_geo_transform[0] += (shape_1) * geo_transform[1]  # East
                label_geo_transform[3] += (shape_0) * geo_transform[5]  # North

            data = image_matrix[shape_0:shape_0 +
                                image_size, shape_1:shape_1 + image_size]
            ...
            name = os.path.split(
                image_filepath)[-1].replace(".tif", "") + f"_n_{shape_0}_e_{shape_1}"
        ...
        for i in range(len(image_filepaths)):
            ...
            for image in training_images:
                ...
                data_path = os.path.join(
                    output_folder, "images", image.name + ".tif")
                image.write_data_to_raster(data_path)
                # Labels
                label_path = os.path.join(
                    output_folder, "labels", image.name + ".tif")
...
def find_closest_pixel(i, j, arrays, threshold=10):
    ...
    for identifier, array in enumerate(arrays):
        if array is not None:
            id_count[identifier] += np.sum(array[i-max_radius:i +
                                           max_radius, j-max_radius:j+max_radius])
    if sum(id_count) > 0:
...
def reassemble_big_image(images, small_image_size=512, big_image_shape=(6000, 8000), unknown_zero=True):
    if unknown_zero:
        big_image = np.zeros(big_image_shape)
    else:
...
def reassemble_big_image_np(images, filling_array, small_image_size=512, big_image_shape=(6000, 8000), unknown_zero=True):
    """
    reasmbles the large image where images are numpy array
    """
    if unknown_zero:
        big_image = np.zeros(big_image_shape)
    else:
    ...
    for idx, image in enumerate(images):
        ...
        image_offset_shape_1 = image.east_offset
        big_image[image_offset_shape_0:image_offset_shape_0+small_image_size,
                  image_offset_shape_1:image_offset_shape_1+small_image_size] = filling_array[idx]
    return big_image
...
def merge_labels_rasters(label_raster_dict):
    ...
    for array in arrays:
        ...
        if array.shape != shape:
            raise Exception(
                f"The shapes does not match, {shape} != {array.shape}")
...
def create_raster_labels(image_path, poly_dict, destination_path, driver=gdal.GetDriverByName("GTiff")):
    ...
    for current_class in poly_dict:
        ...
        if len(intersecting_polys) == 0:
            ...
            shapefile_path = r"D:\temp\post_processed_label_" + \
                os.path.split(destination_path.replace(".tif", ".shp"))[-1]
...
def create_bounding_box(image_ds):
    ...
    ring.AddPoint(top_left_coordinate[0],
                  bottom_right_coordinate[1])  # Top right
    # Bottom right
    ring.AddPoint(bottom_right_coordinate[0], bottom_right_coordinate[1])
    ring.AddPoint(bottom_right_coordinate[0],
                  top_left_coordinate[1])  # Bottom left
    # Top left, closes the ring
    ring.AddPoint(top_left_coordinate[0], top_left_coordinate[1])
...
def convert_many_to_CSV(pointer_file_dir, dest_dir):
    ...
    print(
        f"Converted all images in {pointer_files} to csv \nand saved them in {dest_dir}")
...
def train_valid_test_split(source_folder, dest_folder, train=0.8, valid=0.2, test=0, split_by_big_images=False):
    ...
    if train + valid + test != 1:
        raise ValueError(
            f"train, valid and test must add up to 1. They added up to {train + valid + test}")
    ...
    if split_by_big_images:
        ...
        for image in tiny_images:
            if image[:15] in train_images:
                ...
                raise ValueError(
                    f"No match was found for {image}. All images should have a match.")
...
def divide_and_filter_main():
    ...
    divide_and_save_images(image_paths, label_paths, DEST_ROOT_PATH,
                           image_size=512, do_overlap=False, do_crop=False)
